...
├── finlink_mobile/            # Flutter mobile app
...
The Flutter app lives in `finlink_mobile` and currently includes:

- Auth flow: login and registration
- Wallet actions: deposit, withdraw, and receive via QR
- Loan flow: application screen
- Profile view and notifications surfaces

Routing is handled by `go_router`, and dependencies are wired through `get_it`.

## How to Run It (Local Development)

To run the project locally, you will need **Docker**, **Docker Compose**, and the **Flutter SDK**.
...
Go to the `finlink_mobile` directory, install dependencies, and run Flutter:
```bash
cd finlink_mobile
flutter pub get
flutter run
```
You can run it on your physical device or locally in an iOS Simulator / Android Emulator.
